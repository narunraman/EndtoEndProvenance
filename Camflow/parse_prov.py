...
def get_agent():
        # ag_prefix = 'a1'
        return json.dumps(data['agent'])[1:-1].encode('utf-8')

def get_all_activities():
    activities = data['activity']
    for activity in activities:
        ac_prefix = extract_id(activity)
        activity_i = activities[activity]
        node = json.dumps(activity) + ":" + json.dumps(activity_i)
        yield node, ac_prefix
...
def get_all_entities():
    ...
    for entity in entities:
        ...
        entity_i = json.dumps(entity) + ":" + json.dumps(entities[entity])
        if extract_entity_type(entity) == 'd':
            entity_id = extract_id(entity)
            ...
            feedcamflow.node_environment(entity_i.encode('utf-8'))
